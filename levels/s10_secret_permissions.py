...
echo "__SECRETS__"
for p in {quoted}; do
  if [ -e "$p" ]; then stat -Lc '%a|%U|%G|%n' "$p" 2>/dev/null; else echo "MISSING|$p"; fi
done
'''
    try:
        ...
        return
    if r["exit_code"]!=0:
        report.add("secrets.remote.permissions","INFRA_ERROR","secrets","Could not collect remote permission metadata.",evidence=r)
        return
    out=r["stdout_tail"]
    ...
    fps=out.split("__AUTHORIZED_KEY_FINGERPRINTS__",1)[-1].split("__SECRETS__",1)[0].strip().splitlines()
    ...
    sec=out.split("__SECRETS__",1)[-1].strip().splitlines()
