...
class RecoveryAssistantTests(unittest.TestCase):
    ...
    def test_preflight_rejects_offsite_inside_target(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch.object(core, "effective_config", return_value=cfg), \
                 ...
                 mock.patch.object(core, "_verify_database_runtime", return_value={"db_secret_match": True, "db_connection": True}):
                with self.assertRaises(core.RecoveryError):
    ...
    def test_database_runtime_guard_checks_secret_match_and_connection(self):
        cfg = {
            "remote": {
                "enabled": True,
                "user": "kx-admin",
        ...
        services = {
            "postgres": {"name": "kx-prod-postgres"},
            "django": {"name": "kx-prod-django-api"},
        }
        seen = {}
        ...
        def fake_remote(_cfg, script, **_kwargs):
            ...
            return {
                "exit_code": 0,
                "timed_out": False,
                "stdout_tail": "DB_SECRET_MATCH|true\nDB_CONNECTION|true\n",
                "stderr_tail": "",
            }

        with mock.patch.object(core, "_remote_result", side_effect=fake_remote):
            result = core._verify_database_runtime(cfg, services)

        self.assertTrue(result["db_secret_match"])
        self.assertTrue(result["db_connection"])
        script = seen["script"]
        self.assertIn("hashlib.sha256", script)
        self.assertIn("$POSTGRES_PASSWORD", script)
        self.assertIn("Database credential mismatch", script)
        self.assertIn("connection.ensure_connection", script)
        self.assertNotIn("echo $POSTGRES_PASSWORD", script)

    def test_database_runtime_guard_requires_complete_evidence(self):
        cfg = {"remote": {"user": "root"}}
        services = {
            "postgres": {"name": "pg"},
            "django": {"name": "dj"},
        }
        with mock.patch.object(
            core,
            "_remote_result",
            return_value={"exit_code": 0, "timed_out": False, "stdout_tail": "DB_SECRET_MATCH|true\n", "stderr_tail": ""},
        ):
            with self.assertRaises(core.RecoveryError):
                core._verify_database_runtime(cfg, services)
    ...
    def test_restore_drill_script_is_internal_only_and_writes_attestation(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn('docker rm -fv "$APP" "$REDIS" "$PG"', restore_script)
