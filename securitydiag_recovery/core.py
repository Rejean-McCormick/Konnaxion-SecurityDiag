...
def _verify_database_runtime(cfg: dict, services: dict[str, dict], progress: Progress | None = None) -> dict:
    """Verify that Django and PostgreSQL agree on the effective DB credential.

    Only SHA-256 digests are compared remotely; secret values are never returned.
    The check also proves that PostgreSQL accepts its configured credential and that
    Django can establish a real database connection before a backup is permitted.
    """
...
PG={pg}
DJ={dj}
DJ_HASH=$(docker exec "$DJ" python /app/manage.py shell -c 'from django.conf import settings; import hashlib; p=str(settings.DATABASES["default"].get("PASSWORD","")); print(hashlib.sha256(p.encode()).hexdigest())' 2>/dev/null | tail -n 1 | tr -d '[:space:]')
PG_HASH=$(docker exec "$PG" sh -lc 'printf "%s" "$POSTGRES_PASSWORD" | sha256sum | cut -d" " -f1' | tail -n 1 | tr -d '[:space:]')
case "$DJ_HASH" in ''|*[!0-9a-f]*) echo 'Unable to fingerprint Django database credential.' >&2; exit 41;; esac
case "$PG_HASH" in ''|*[!0-9a-f]*) echo 'Unable to fingerprint PostgreSQL database credential.' >&2; exit 42;; esac
[ "${{#DJ_HASH}}" -eq 64 ] || {{ echo 'Unexpected Django credential fingerprint length.' >&2; exit 43; }}
[ "${{#PG_HASH}}" -eq 64 ] || {{ echo 'Unexpected PostgreSQL credential fingerprint length.' >&2; exit 44; }}
if [ "$DJ_HASH" != "$PG_HASH" ]; then
  echo 'Database credential mismatch: Django and PostgreSQL do not use the same effective password.' >&2
  exit 45
fi
docker exec "$PG" sh -lc 'PGPASSWORD="$POSTGRES_PASSWORD" psql -h 127.0.0.1 -At -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "select 1"' >/dev/null
docker exec "$DJ" python /app/manage.py shell -c 'from django.db import connection; connection.ensure_connection()' >/dev/null 2>&1
printf 'DB_SECRET_MATCH|true\n'
printf 'DB_CONNECTION|true\n'
"""
    r = _remote_result(cfg, script, timeout=90, privileged=True)
    evidence = {
        line.strip().split("|", 1)[0]: line.strip().split("|", 1)[1]
        for line in r.get("stdout_tail", "").splitlines()
        if "|" in line
    }
    if evidence.get("DB_SECRET_MATCH") != "true" or evidence.get("DB_CONNECTION") != "true":
        raise RecoveryError("Database runtime verification did not return complete evidence.")
    _emit(progress, "Database runtime verified: Django/PostgreSQL credentials match and DB connection succeeds.")
    return {"db_secret_match": True, "db_connection": True}
...
def preflight(tool_root: Path | str, offsite_dir: Path | str | None = None, progress: Progress | None = None) -> dict:
    ...
    db_runtime = _verify_database_runtime(cfg, services, progress)
...
"""
    r = _remote_result(cfg, script, timeout=60, privileged=True)
    _emit(progress, "Preflight passed: SSH/sudo, backup path, env files, Docker runtime and database authentication are available.")
    return {
        "target": str(target),
        "backup_root": root,
        "env_paths": envs,
...
cleanup() {{
  docker rm -fv "$APP" "$REDIS" "$PG" >/dev/null 2>&1 || true
...
"""
        ...
        try:
            cleanup = (
                f"docker rm -fv {shlex.quote(app)} {shlex.quote(redis)} {shlex.quote(pg)} >/dev/null 2>&1 || true\n"
