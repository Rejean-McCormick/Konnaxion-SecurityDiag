...
"""
    ...
    for line in r.get("stdout_tail", "").splitlines():
        ...
        if not all(SAFE_NAME.fullmatch(v) for v in (svc, name, image_ref, image_id)):
            continue
        services[svc] = {"name": name, "image_ref": image_ref, "image_id": image_id}
    missing = [x for x in ("postgres", "django", "redis") if x not in services]
    if missing:
        raise RecoveryError(f"Running production service(s) not found: {', '.join(missing)}")
    return services
